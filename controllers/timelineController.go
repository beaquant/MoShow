...
//NewCommers .
// @Title 首页专区列表
// @Description newcomer:新用户专区,注册时间最近15天内的用户在此参与排序。(性别为1时，查询所有男用户,为0时，只查女主播，不查普通女用户) active:活跃专区,所有用户参与查询。(性别为1时，查询所有男用户,为0时，只查女主播，不查普通女用户) suggestion:推荐专区,用户由运营人员推荐
